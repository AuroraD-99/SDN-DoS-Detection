...
class BlocklistApi(ControllerBase):
    ...
    def add_to_blocklist(self, req, **kwargs):
        try:
            ...
            ip_src = body.get('ip_address') 
            ip_dst = body.get('ip_dst')     
...
class HostApi(ControllerBase):
            ...
            for dpid, mac_info_by_mac in self.mac_to_ip.items():
                ...
                for mac, info in mac_info_by_mac.items():
                    ...
                    hosts_info_list.append({
                        ...
                        'current_host_total_bandwidth_mbps': host_flow_rates['total_bandwidth_mbps'], 
                        'current_host_total_packet_rate_pps': host_flow_rates['total_packet_rate_pps'], 
    ...
    def get_host_details(self, req, ip_address, **kwargs): 
        try:
            if not ip_address:
                ...
                for mac, info in mac_info_by_mac.items():
                    if info.get('ip_src') == ip_address:
                        ...
                        host_details = {
                            ...
                            'total_flow_bandwidth_mbps': 0.0, 
                            'total_flow_packet_rate_pps': 0.0, 
                            'flows': [] 
                        }
                        ...
                        with self.flow_stats_lock:
                            if dpid in self.flow_stats:
                                for flow_key_tuple, flow_info in self.flow_stats[dpid].items():
                                    ...
                                    if ip_src_flow == ip_address or ip_dst_flow == ip_address: 
